...
*************************************************************************************************************************************************************************************/
/**********************RUN THE FOLLOWING FIRST QUERY IN CARTO BATCH******************************/
select
	*
into
	hpd_dob_match
...
(
	select
		...
		case when 	st_distance(cast(a.the_geom as geography),cast(d.the_geom as geography)) > 0 and 
				  	st_distance(cast(a.the_geom as geography),cast(d.the_geom as geography)) < 20 	then 1 end 	as proximity_match,
	...
	on
		...
		/*and coalesce(a.total_units,0) - coalesce(d.units_net,0) >= -30*/ /*Omitting matches where DOB jobs are unconfirmed (not matched by address or bbl) and
																	 substantially larger than HPD jobs. If a DOB job is smaller,
																	 it could be a portion of the HPD job.*/
	where a.source = 'HPD Projects'
...
(
	select
		...
		case 
			...
			when proximity_match 	= 1	then 'Proximity' end as DOB_Match_Type,
		case 
			when concat(project_id,', ',dob_job_number) in
													(
														select
														...
														else dob_job_number end as dob_job_number,
		case 
			when concat(project_id,', ',dob_job_number) in
													(
														select
														...
														else units_net end as DOB_Units_Net,
		address,
		borough,
		latitude,
		longitude,
