...
drop table if exists aggregated_ct_cp_assumptions;
drop table if exists ungeocoded_PROJECTs_ct_cp_assumptions;
drop table if exists aggregated_ct_longform_cp_assumptions;
drop table if exists aggregated_ct_PROJECT_level_cp_assumptions;

select
	*
into
	aggregated_ct_cp_assumptions
from
(
...
select
	*
into
	ungeocoded_projects_ct_cp_assumptions
from
(
...
(
	...
	from
		aggregated_ct_cp_assumptions a 
	left join
		capitalplanning.census_tract_2010_190412_ms b
	on 
...
select
	*
into
	aggregated_ct_longform_cp_assumptions
from
(
...
select
	*
into
	aggregated_ct_project_level_cp_assumptions
from
(
	SELECT
		the_geom,
...
drop table if exists longform_ct_output_cp_assumptions;
SELECT
	*
into
	longform_ct_output_cp_assumptions
from
(
	SELECT 
		*  
	FROM 
		capitalplanning.aggregated_ct_longform_cp_assumptions 
	where 
...
select cdb_cartodbfytable('capitalplanning','longform_ct_output_cp_assumptions');


drop table if exists longform_ct_output_cp_assumptions_mta;
SELECT
	*
into
	longform_ct_output_cp_assumptions_mta
from
(
	SELECT 
		*  
	...
	where 
		(
			not (source = 'DOB' and status in('Complete','Complete (demolition)')) and
			source not in('HPD Projected Closings','Future Neighborhood Studies')
		)
			or 
		(
			source = 'Future Neighborhood Studies' and project_id = 'Gowanus Future Rezoning Development'
		) 
) x;

select cdb_cartodbfytable('capitalplanning','longform_ct_output_cp_assumptions_mta');
